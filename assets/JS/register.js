...
function updateProgress() {
  const fields = ["fullname", "licence_number", "phone", "email", "password"];
...
async function handleRegister(event) {
  ...
  const formData = {
    ...
    licence_number: document.getElementById("licence_number") ? document.getElementById("licence_number").value.trim() : '',
...
// Client-side validation for licence and phone before native submit
function validateLicenceAndPhone() {
  const licence = document.getElementById('licence_number').value.trim();
  const phone = document.getElementById('phone').value.trim();

  const licenceRe = /^[A-Za-z0-9]{16}$/;
  const phoneRe = /^\d{10}$/;

  if (!licenceRe.test(licence)) {
    Swal.fire({icon: 'error', title: 'Invalid Licence', text: 'Licence number must be 16 alphanumeric characters.'});
    return false;
  }
...
// Attach a submit listener to the form to perform client-side validation
document.addEventListener('DOMContentLoaded', function () {
  const form = document.querySelector('form');
  if (!form) return;

  form.addEventListener('submit', function (e) {
    // Run our validations and stop submission if invalid
    if (!validateLicenceAndPhone()) {
      e.preventDefault();
      return false;
    }
    // Otherwise allow native submission to proceed (server-side will also validate)
  });
});
